...
class TICoordinatorAsync:
    ...
    async def initialize_async(self) -> bool:
        ...
        try:
            # 1. Criar agente TI base (em thread separada para não bloquear)
            ...
            sub_agents_tasks = [
                asyncio.to_thread(criar_agente_governance, debug=self.debug),
                asyncio.to_thread(criar_agente_infra, debug=self.debug),
                asyncio.to_thread(criar_agente_dev, debug=self.debug),
                asyncio.to_thread(criar_agente_enduser, debug=self.debug)
            ]
            
            sub_agents_results = await asyncio.gather(*sub_agents_tasks, return_exceptions=True)
            
            # Nomes dos sub-agentes para logging
            sub_agent_names = ['Governança', 'Infraestrutura', 'Desenvolvimento', 'End-User']
