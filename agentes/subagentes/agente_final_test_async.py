"""Agente especializado em Teste de Integração Completa - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Final Test, um especialista em Teste de Integração Completa. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Teste de Integração Completa.
...
RESPOSTA (considere teste de integração completa e histórico ao responder):
    """
).strip()

KEYWORDS = ['teste', 'final', 'validacao']


class Agentefinal_testAsync:
    """Agente especializado em Teste de Integração Completa com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="final_test",
            name="Final Test",
            specialty="Teste de Integração Completa",
            description="Agente para validar correção do singleton",
            keywords=KEYWORDS,
            table_name="knowledge_final_test",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre teste de integração completa. Por favor, tente novamente."
...
def criar_agente_final_test_async(*, debug: bool = False) -> Agentefinal_testAsync:
    """Factory function para criar o agente de Teste de Integração Completa assíncrono"""
    return Agentefinal_testAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_final_test_async():
        agente = criar_agente_final_test_async(debug=True)
        
        perfil_demo = {
            "Nome": "Teste Usuario",
            "Cargo": "Analista",
            "Departamento": "Teste de Integração Completa",
            "nivel_hierarquico": 2,
            "geografia": "BR",
            "projetos": ["N/A"],
        }
        ...
        perguntas_demo = [
            "Teste de pergunta para Teste de Integração Completa",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 Final Test: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_final_test_async())
