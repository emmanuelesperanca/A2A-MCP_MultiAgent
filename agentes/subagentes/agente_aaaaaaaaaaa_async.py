"""Agente especializado em aaaaa - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é aaaaaaaaaaa, um especialista em aaaaa. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à aaaaa.
...
RESPOSTA (considere aaaaa e histórico ao responder):
    """
).strip()

KEYWORDS = ['aaaaaa', 'aaaaa', 'a']


class AgenteaaaaaaaaaaaAsync:
    """Agente especializado em aaaaa com operações ASSÍNCRONAS"""
    ...
    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            ...
            keywords=KEYWORDS,
            table_name="knowledge_aaaaaaaaaaa",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre aaaaa. Por favor, tente novamente."
...
def criar_agente_aaaaaaaaaaa_async(*, debug: bool = False) -> AgenteaaaaaaaaaaaAsync:
    """Factory function para criar o agente de aaaaa assíncrono"""
    return AgenteaaaaaaaaaaaAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_aaaaaaaaaaa_async():
        agente = criar_agente_aaaaaaaaaaa_async(debug=True)
        ...
        perguntas_demo = [
            "Teste de pergunta para aaaaa",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 aaaaaaaaaaa: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_aaaaaaaaaaa_async())
