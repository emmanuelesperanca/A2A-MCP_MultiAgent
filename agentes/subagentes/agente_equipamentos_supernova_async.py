"""Agente especializado em Teste - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Vane, um especialista em Teste. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Teste.
...
RESPOSTA (considere teste e histórico ao responder):
    """
).strip()

KEYWORDS = ['equipamentos', 'teste']


class Agenteequipamentos_supernovaAsync:
    """Agente especializado em Teste com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="equipamentos_supernova",
            name="Vane",
            specialty="Teste",
            description="Teste",
            keywords=KEYWORDS,
            table_name="knowledge_equipamentos_supernova",
...
def criar_agente_equipamentos_supernova_async(*, debug: bool = False) -> Agenteequipamentos_supernovaAsync:
    """Factory function para criar o agente de Teste assíncrono"""
    return Agenteequipamentos_supernovaAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_equipamentos_supernova_async():
        agente = criar_agente_equipamentos_supernova_async(debug=True)
        ...
        for pergunta in perguntas_demo:
            ...
            print(f"🤖 Vane: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_equipamentos_supernova_async())
