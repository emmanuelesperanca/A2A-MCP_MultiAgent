"""Agente especializado em Suporte e manutenção de equipamentos e máquinas da fábrica - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Alexandre, um especialista em Suporte e manutenção de equipamentos e máquinas da fábrica. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Suporte e manutenção de equipamentos e máquinas da fábrica.
...
RESPOSTA (considere suporte e manutenção de equipamentos e máquinas da fábrica e histórico ao responder):
    """
).strip()

KEYWORDS = ['equipamentos', 'manutenção', 'máquinas', 'fábrica']


class AgenteequipamentosAsync:
    """Agente especializado em Suporte e manutenção de equipamentos e máquinas da fábrica com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="equipamentos",
            name="Alexandre",
            specialty="Suporte e manutenção de equipamentos e máquinas da fábrica",
            description="Agente especialista em dar suporte, manutenção para equipamentos e máquinas da fábrica da Neodent.",
            keywords=KEYWORDS,
            table_name="knowledge_equipamentos",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre suporte e manutenção de equipamentos e máquinas da fábrica. Por favor, tente novamente."
...
def criar_agente_equipamentos_async(*, debug: bool = False) -> AgenteequipamentosAsync:
    """Factory function para criar o agente de Suporte e manutenção de equipamentos e máquinas da fábrica assíncrono"""
    return AgenteequipamentosAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_equipamentos_async():
        agente = criar_agente_equipamentos_async(debug=True)
        
        perfil_demo = {
            "Nome": "Teste Usuario",
            "Cargo": "Analista",
            "Departamento": "Suporte e manutenção de equipamentos e máquinas da fábrica",
            "nivel_hierarquico": 2,
            "geografia": "BR",
            "projetos": ["N/A"],
        }
        ...
        perguntas_demo = [
            "Teste de pergunta para Suporte e manutenção de equipamentos e máquinas da fábrica",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 Alexandre: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_equipamentos_async())
