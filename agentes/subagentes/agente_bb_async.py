"""Agente especializado em bbb - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é bbbb, um especialista em bbb. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à bbb.
...
RESPOSTA (considere bbb e histórico ao responder):
    """
).strip()
...
class AgentebbAsync:
    """Agente especializado em bbb com operações ASSÍNCRONAS"""
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre bbb. Por favor, tente novamente."
...
def criar_agente_bb_async(*, debug: bool = False) -> AgentebbAsync:
    """Factory function para criar o agente de bbb assíncrono"""
    return AgentebbAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_bb_async():
        agente = criar_agente_bb_async(debug=True)
