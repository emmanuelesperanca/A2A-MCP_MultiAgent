"""Agente especializado em string - VERSÃO ASSÍNCRONA
...
class AgentestringAsync:
    """Agente especializado em string com operações ASSÍNCRONAS"""
    ...
    def __init__(self, *, debug: bool = False) -> None:
        ...
        self.mcp_tools_category = "string"
        self.allowed_tools = ['string']
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre string. Por favor, tente novamente."
...
def criar_agente_string_async(*, debug: bool = False) -> AgentestringAsync:
    """Factory function para criar o agente de string assíncrono"""
    return AgentestringAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_string_async():
        agente = criar_agente_string_async(debug=True)
        ...
        perguntas_demo = [
            "Teste de pergunta para string",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 string: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_string_async())
