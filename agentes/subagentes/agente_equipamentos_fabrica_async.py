"""Agente especializado em Suporte e Manutenção para os Equipamentos da Fábrica - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Aline, um especialista em Suporte e Manutenção para os Equipamentos da Fábrica. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Suporte e Manutenção para os Equipamentos da Fábrica.
...
RESPOSTA (considere suporte e manutenção para os equipamentos da fábrica e histórico ao responder):
    """
).strip()

KEYWORDS = ['equipamentos', 'fábrica', 'manutenção']


class Agenteequipamentos_fabricaAsync:
    """Agente especializado em Suporte e Manutenção para os Equipamentos da Fábrica com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="equipamentos_fabrica",
            name="Aline",
            specialty="Suporte e Manutenção para os Equipamentos da Fábrica",
            description="Suporte e Manutenção para os Equipamentos da Fábrica",
            keywords=KEYWORDS,
            table_name="knowledge_equipamentos_fabrica",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre suporte e manutenção para os equipamentos da fábrica. Por favor, tente novamente."
...
def criar_agente_equipamentos_fabrica_async(*, debug: bool = False) -> Agenteequipamentos_fabricaAsync:
    """Factory function para criar o agente de Suporte e Manutenção para os Equipamentos da Fábrica assíncrono"""
    return Agenteequipamentos_fabricaAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_equipamentos_fabrica_async():
        agente = criar_agente_equipamentos_fabrica_async(debug=True)
        
        perfil_demo = {
            "Nome": "Teste Usuario",
            "Cargo": "Analista",
            "Departamento": "Suporte e Manutenção para os Equipamentos da Fábrica",
            "nivel_hierarquico": 2,
            "geografia": "BR",
            "projetos": ["N/A"],
        }
        ...
        perguntas_demo = [
            "Teste de pergunta para Suporte e Manutenção para os Equipamentos da Fábrica",
        ]
    ...
    asyncio.run(test_agente_equipamentos_fabrica_async())
