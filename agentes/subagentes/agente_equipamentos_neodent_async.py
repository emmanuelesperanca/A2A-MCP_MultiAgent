"""Agente especializado em Manutenção e Suporte de equipamentos e máquinas. - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Nathalia, um especialista em Manutenção e Suporte de equipamentos e máquinas.. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Manutenção e Suporte de equipamentos e máquinas..
...
RESPOSTA (considere manutenção e suporte de equipamentos e máquinas. e histórico ao responder):
    """
).strip()

KEYWORDS = ['equipamentos', 'manutenção', 'maquinas']


class Agenteequipamentos_neodentAsync:
    """Agente especializado em Manutenção e Suporte de equipamentos e máquinas. com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="equipamentos_neodent",
            name="Nathalia",
            specialty="Manutenção e Suporte de equipamentos e máquinas.",
            description="Manutenção e Suporte de equipamentos e máquinas da fabrica da Neodent.",
            keywords=KEYWORDS,
            table_name="knowledge_equipamentos_neodent",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre manutenção e suporte de equipamentos e máquinas.. Por favor, tente novamente."
...
def criar_agente_equipamentos_neodent_async(*, debug: bool = False) -> Agenteequipamentos_neodentAsync:
    """Factory function para criar o agente de Manutenção e Suporte de equipamentos e máquinas. assíncrono"""
    return Agenteequipamentos_neodentAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_equipamentos_neodent_async():
        agente = criar_agente_equipamentos_neodent_async(debug=True)
        
        perfil_demo = {
            "Nome": "Teste Usuario",
            "Cargo": "Analista",
            "Departamento": "Manutenção e Suporte de equipamentos e máquinas.",
            "nivel_hierarquico": 2,
            "geografia": "BR",
            "projetos": ["N/A"],
        }
        ...
        perguntas_demo = [
            "Teste de pergunta para Manutenção e Suporte de equipamentos e máquinas.",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 Nathalia: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_equipamentos_neodent_async())
