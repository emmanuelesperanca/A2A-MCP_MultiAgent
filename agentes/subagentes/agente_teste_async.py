"""Agente especializado em Fazer testes - VERSÃO ASSÍNCRONA
...
PROMPT_TEMPLATE = dedent(
    """
Você é Teste, um especialista em Fazer testes. Você faz parte do ecossistema Neoson e orienta colaboradores internos com dúvidas relacionadas à Fazer testes.
...
RESPOSTA (considere fazer testes e histórico ao responder):
    """
).strip()
...
class AgentetesteAsync:
    """Agente especializado em Fazer testes com operações ASSÍNCRONAS"""

    def __init__(self, *, debug: bool = False) -> None:
        self.config = SubagentConfig(
            identifier="teste",
            name="Teste",
            specialty="Fazer testes",
            description="Especialdiadnsaon sdoadj em e testes",
            keywords=KEYWORDS,
            table_name="knowledge_teste",
    ...
    async def processar_async(self, pergunta: str, user_profile: dict) -> str:
        ...
        try:
            ...
            print(error_msg)
            return "Desculpe, encontrei um erro ao processar sua pergunta sobre fazer testes. Por favor, tente novamente."
...
def criar_agente_teste_async(*, debug: bool = False) -> AgentetesteAsync:
    """Factory function para criar o agente de Fazer testes assíncrono"""
    return AgentetesteAsync(debug=debug)


if __name__ == "__main__":
    import asyncio
    
    async def test_agente_teste_async():
        agente = criar_agente_teste_async(debug=True)
        
        perfil_demo = {
            "Nome": "Teste Usuario",
            "Cargo": "Analista",
            "Departamento": "Fazer testes",
            "nivel_hierarquico": 2,
            "geografia": "BR",
            "projetos": ["N/A"],
        }
        ...
        perguntas_demo = [
            "Teste de pergunta para Fazer testes",
        ]

        for pergunta in perguntas_demo:
            print(f"\n❓ Pergunta: {pergunta}")
            resposta = await agente.processar_async(pergunta, perfil_demo)
            print(f"🤖 Teste: {resposta}")
            print("-" * 80)
    
    asyncio.run(test_agente_teste_async())
